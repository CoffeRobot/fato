...
{
public:
    Tracker3D(int thresh = 30, int ocataves = 3, float patternScale = 1.0f) :
		m_threshold(thresh),
		m_octave(ocataves),
...
private:
	void getCurrentPoints(const vector<int>& currentFaces, vector<Status*>& pointsStatus, 
		vector<Point3f*>& fstPoints, vector<Point3f*>& updPoints, vector<KeyPoint*>& fstKeypoints,
		vector<KeyPoint*>& updKeypoints, vector<Point3f*>& relPointPos, Mat& fstDescriptors,
	...
	int matchFeaturesCustom(const Mat& fstDescriptors,
		const vector<KeyPoint*>& fstKeypoints, const Mat& nextDescriptors, 
		const vector<KeyPoint>& extractedKeypoints, vector<KeyPoint*>& updKeypoints,
		vector<Status*>& pointsStatus );
	// uses lucas kanade to track keypoints, faster implemetation
	int trackFeatures(const Mat& grayImg, const Mat& cloud, vector<Status*>& keypointStatus, 
		vector<Point3f*>& updPoints, vector<KeyPoint*>& updKeypoints, 
		int& trackedCount, int& bothCount);
	...
	cv::Mat getRotationMatrix(const vector<Point3f*>& fstPoints,
		const vector<Point3f*>& updPoints, const vector<Status*>& pointsStatus);

	cv::Mat getRotationMatrixDebug(const Mat& rgbImg, const vector<Point3f*>& fstPoints,
		const vector<Point3f*>& updPoints, const vector<Status*>& pointsStatus);
	// given rotation and scale cast votes for the new centroid
	void getVotes(const cv::Mat& cloud, vector<Status*> pointsStatus, vector<KeyPoint*>& fstKeypoints,
		vector<KeyPoint*>& updKeypoints, vector<Point3f*>& relPointPos, const Mat& rotation);
	// cluster the votes
	void clusterVotes(vector<Status>& keypointStatus);

	void clusterVotesBorder(vector<Status*>& keypointStatus,
		vector<Point3f>& centroidVotes, vector<int>& indices, vector<vector<int>>& clusters);
	...
	void updateCentroid(const vector<Status*>& keypointStatus, const Mat& rotation);

	void updatePointsStatus(vector<Status*>& pointsStatus, vector<bool>& isClustered);

	void initBBox(const Mat& cloud);

	bool isAppearanceNew(const Mat& rotation);

	bool isCurrentApperanceToLearn(const vector<float>& visibilityRatio, const double& medianAngle, 
		int& faceToLearn);

	bool learnFrame(const Mat& rgb, const Mat& cloud, const vector<bool>& isFaceVisible,
		const vector<float>& visibilityRatio, const Mat& rotation);

	void learnFrame(const Mat& rgb, const Mat& cloud, const int& faceToLearn, const Mat& rotation);

	void learnFace(const Mat1b& mask, const Mat& rgb, const Mat& cloud, const Mat& rotation,
		const int& face, BorgCube& fstCube, BorgCube& updatedCube);

	int learnFaceDebug(const Mat1b& mask, const Mat& rgb, const Mat& cloud, const Mat& rotation,
		const int& face, BorgCube& fstCube, BorgCube& updatedCube, Mat& out);

	void debugTrackingStep(const cv::Mat& fstFrame, const cv::Mat& scdFrame,
		const std::vector<int>& indices, std::vector<vector<int> >& clusters, const bool isLost,
		const bool isLearning, const vector<Status*>& pointsStatus, vector<KeyPoint*>& fstKeypoints,
		...
		std::vector<bool> visibleFaces, std::vector<float>& visibilityRatio, double angularDistance,
		Mat& out);

	void debugTrackingStepICRA(const cv::Mat& fstFrame, const cv::Mat& scdFrame,
		const std::vector<int>& indices, std::vector<vector<int> >& clusters, const bool isLost,
		const bool isLearning, const vector<Status*>& pointsStatus, vector<KeyPoint*>& fstKeypoints,
		const vector<Point3f*>& fstPoints, const vector<Point3f*>& updPoints,
		const vector<KeyPoint*>& updKeypoints, const vector<Scalar*>& colors,
	...
	{
		return (s == Status::MATCH || s == Status::TRACK || s == Status::INIT || s == Status::BOTH);
	}
	...
	void calculateVisibility(const Mat& rotation, const BorgCube& fstCube,
		vector<bool>& isFaceVisible, vector<float>& visibilityRatio);
	...
	bool findObject(BorgCube& fst, BorgCube& upd, const Mat& extractedDescriptors,
		const vector<KeyPoint>& extractedKeypoints, int& faceFound, int& matchesNum);
