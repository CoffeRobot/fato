...
void Tracker2::init(Mat& src, Mat& mask)
{
	...
	cout << "Feature descriptor size: " << m_firstFrameDescriptors.size() << std::endl;

	if (m_configFile.m_useSysthetic)
		extractSyntheticKeypoints(m_fstFrame, keypoints, m_firstFrameDescriptors);
	...
	for (int kp = 0; kp < keypoints.size(); ++kp)
	{
		...
		m_pointsColor.push_back(
			Scalar(uniform_dist(engine), uniform_dist(engine), uniform_dist(engine)));
	}
	...
	m_timeFile.open(m_configFile.m_dstPath + "profilingV2.txt");
	//m_debugFile << "AS MS AR MR CX CY Vkp\n";

	m_debugWriter.open(m_configFile.m_dstPath + m_configFile.m_dstFilename,
		CV_FOURCC('X', 'V', 'I', 'D'), 30, Size(1280, 480), true);

	m_extractT = m_matchT = m_trackT = m_scaleT = m_rotT = m_votingT = m_updateT = m_clusterT =
		m_centroidT = m_drawT = 0;
	m_updateVoteT = 0;
	m_numFrames = 0;
	m_clusterVoteSize = 0;

	// adding to the set of learned frames the initial frame with angle 0 and scale 1
	m_learnedFrames.insert(pair<int, int>(0, 0));
}

void Tracker2::extractSyntheticKeypoints(const cv::Mat& src, std::vector<cv::KeyPoint>& points,
	cv::Mat& descriptors)
{
	...
	for (size_t i = 0; i < kernelSize; i++)
	{
		...
		{
			if (i == j)
				tr.at<float>(i, j) = elemVal;
			if (i == kernelSize / 2 + 1)
				hk.at<float>(i, j) = elemVal;
			if (j == kernelSize / 2 + 1)
				vk.at<float>(i, j) = elemVal;
			if (i == kernelSize - j - 1)
				br.at<float>(i, j) = elemVal;
		}
	}
...
void Tracker2::computeNext(Mat& next, Mat& out)
{
	...
	start = chrono::system_clock::now();
	if (m_configFile.m_filterBorder)
		clusterVotesBorder(indices, clusters);
	else
		clusterVotesDebug(indices, clusters);
	end = chrono::system_clock::now();
	m_clusterT += chrono::duration_cast<chrono::milliseconds>(end - start).count();
	cout << "7 ";
	/*********************************************************************************************/
	/*                             UPDATE VOTES                                                  */
	/*********************************************************************************************/
	start = chrono::system_clock::now();
	if (m_configFile.m_useUpdate)
		updateVotes();
	end = chrono::system_clock::now();
	m_updateVoteT += chrono::duration_cast<chrono::milliseconds>(end - start).count();
	cout << "8 ";
	/*********************************************************************************************/
	...
	m_centroidT += chrono::duration_cast<chrono::milliseconds>(end - start).count();
	cout << "9 ";
	...
	if (m_configFile.m_useCustomMatcher)
	{
		...
		m_extractT += chrono::duration_cast<chrono::milliseconds>(end - start).count();
		start = chrono::system_clock::now();
	...
	/*********************************************************************************************/
	start = chrono::system_clock::now();
	//auto ftr = async(launch::async, &Tracker::debugTrackingStep, this, grayImg, m_fstFrame);
	if (m_configFile.m_useMulticore)
		parDebugtrackingStep(grayImg, m_fstFrame);
	else
	...
	/*********************************************************************************************/
	if (m_configFile.m_showLearnCandidates)
	{
		learnKeypoints(numMatches, numTracked, numBoth, grayImg, next, nextKeypoints);
	}
...
void Tracker2::close()
{
	...
	float totTime = m_extractT + m_matchT + m_trackT + m_rotT + m_scaleT + m_updateT + m_votingT 
...
void Tracker2::matchFeatures(const Mat& grayImg)
{
	vector<KeyPoint> nextKeypoints;
	...
	for (size_t i = 0; i < matches.size(); i++)
	{
		...
		if (queryId < 0 && queryId >= nextKeypoints.size())
			continue;

		if (trainId < 0 && trainId >= m_firstFrameKeypoints.size())
			continue;
...
int Tracker2::matchFeaturesCustom(const Mat& grayImg, vector<KeyPoint>& nextKeypoints,
	Mat& nextDescriptors)
{
...
void Tracker2::trackFeatures(const Mat& grayImg, int& trackedCount, int& bothCount)
{
	...
	{
		if (m_keypointStatus[i] == Status::TRACK || m_keypointStatus[i] == Status::BOTH ||
			m_keypointStatus[i] == Status::MATCH || m_keypointStatus[i] == Status::INIT)
		{
	...
	calcOpticalFlowPyrLK(m_currFrame, grayImg, currPoints,
		nextPoints, m_status, m_errors);

	calcOpticalFlowPyrLK(grayImg, m_currFrame, nextPoints, prevPoints, m_status, prevErrors);
...
void Tracker2::checkImage(const Mat& src, Mat& gray)
{
	if (src.channels() > 1)
...
float Tracker2::getMedianRotation()
{
...
float Tracker2::getMedianScale()
{
	...
	for (size_t i = 0; i < m_updatedKeypoints.size(); ++i)
	{
		for (size_t j = 0; j < m_firstFrameKeypoints.size(); j++)
		{
			...
			{
				float nextDistance = getDistance(m_updatedKeypoints[i], m_updatedKeypoints[j]);
				float currDistance = getDistance(m_firstFrameKeypoints[i],
...
void Tracker2::getVotes(float scale, float angle)
{
	Mat2f rotMat(2, 2);
...
void Tracker2::clusterVotesDebug(vector<int>& indices, vector<vector<int>>& clusters)
{
...
void Tracker2::clusterVotesBorder(vector<int>& indices, vector<vector<int>>& clusters)
{
	DBScanClustering<Point2f*> clusterer;
...
void Tracker2::initCentroid(vector<KeyPoint>& keypoints)
{
	m_firstCentroid.x = 0;
	m_firstCentroid.y = 0;
...
void Tracker2::initRelativePosition()
{
...
void Tracker2::calculateCentroid(vector<KeyPoint>& keypoints, float scale, float angle, Point2f& p)
{
	p.x = 0;
	p.y = 0;
...
void Tracker2::updateCentroid()
{
	...
	{
		m_updatedBBPoints[i] = m_updatedCentroid +
			m_mScale * mult(rotMat, m_fstRelativeBBPoints[i]);
	}

}

void Tracker2::getClusteredEstimations(vector<float>& scales, vector<float>& angles)
{
...
void Tracker2::updateVotes()
{
	...
	for (size_t i = 0; i < iterNum; i++)
	{
		...
		//m_debugFile << "Iter: " << i <<" scale: " << medianScale << " angle: " << m_mAngle << "\n";
	}
...
void Tracker2::updateVotesDebug(vector<int>& indices, vector<vector<int>>& clusters)
{
	int iterNum = 5;
	...
	for (size_t i = 0; i < iterNum; i++)
	{
		...
		//m_debugFile << "Iter: " << i << " values: " << medianScale << " " << medianAngle
		//	<< " points " << angles.size() << "\n";
	}
...
bool Tracker2::learnFrame(float scale, float angle)
{
	float scaleChange = scale - 1;
	float angleChange = angle;

	int scaleId = static_cast<int>(scaleChange / m_configFile.m_scaleLearnStep);
	int angleId = static_cast<int>(angleChange / m_configFile.m_angleLearnStep);
...
void Tracker2::addLearnedFrame(float scale, float angle)
{
	float scaleChange = scale - 1;
	float angleChange = angle;
...
void Tracker2::learnKeypoints(const int numMatches, const int numTracked, const int numBoth, 
	const Mat& grayImg, const Mat& next, const std::vector<cv::KeyPoint>& keypoints)
{
	...
	if (learnFrame(m_mScale, m_mAngle) && m_validKeypoints > (m_initKPCount * 0.1) &&
		confidence > m_configFile.m_matchConfidence)
	{
		Mat tmp, out;
		int rows = next.rows;
		...
		drawTriangle(m_updatedBBPoints[0], m_updatedBBPoints[1], m_updatedBBPoints[2],
			Scalar(0, 255, 0), 0.5, out);
		drawTriangle(m_updatedBBPoints[0], m_updatedBBPoints[2], m_updatedBBPoints[3],
			Scalar(0, 0, 255), 0.5, out);
		drawTriangleMask(m_updatedBBPoints[0], m_updatedBBPoints[1], m_updatedBBPoints[2], mask);
		drawTriangleMask(m_updatedBBPoints[0], m_updatedBBPoints[2], m_updatedBBPoints[3], mask);

		line(out, m_updatedBBPoints[0], m_updatedBBPoints[1], Scalar(255, 0, 0), 2);
		line(out, m_updatedBBPoints[1], m_updatedBBPoints[2], Scalar(255, 0, 0), 2);
		line(out, m_updatedBBPoints[2], m_updatedBBPoints[3], Scalar(255, 0, 0), 2);
		line(out, m_updatedBBPoints[3], m_updatedBBPoints[0], Scalar(255, 0, 0), 2);

		int kpCount = 0;
		for (int i = 0; i < keypoints.size(); i++)
		{
			if (mask.at<uchar>(keypoints[i].pt) == static_cast<uchar>(255))
			{
				kpCount++;
				//circle(out, keypoints[i].pt, 2, Scalar(255, 0, 0), 1);
		...
		cout << "Building composite image: " << out.channels() << " " << fst.channels() << "\n";
		buildCompositeImg(out, fst, result);

		backprojectKeypoins(newPoints, result);

		int scaleId = static_cast<int>((m_mScale - 1) / m_configFile.m_scaleLearnStep);
		int angleId = static_cast<int>((m_mAngle) / m_configFile.m_angleLearnStep);
...
void Tracker2::backprojectKeypoins(const vector<const KeyPoint*>& points, Mat& out)
{
	Mat2f rotMat(2, 2);
	...
	for (size_t i = 0; i < points.size(); i++)
	{
		...
		if (offSetpoint.x < 0 || offSetpoint.x >= out.cols)
			continue;
		if (offSetpoint.y < 0 || offSetpoint.y >= out.rows)
			continue;

		Scalar color = Scalar(uniform_dist(engine), uniform_dist(engine), uniform_dist(engine));
...
void Tracker2::debugTrackingStep(const cv::Mat& fstFrame, const cv::Mat& scdFrame,
	const std::vector<int>& indices, std::vector<vector<int> >& clusters, Mat& out)
{
	...
	{
		drawObjectLocation(m_firstCentroid, m_fstBBPoints, m_updatedCentroid, m_updatedBBPoints,
			out);
	}

	if (m_configFile.m_showMatching)
	{
		drawKeypointsMatching(m_firstFrameKeypoints, m_updatedKeypoints, m_keypointStatus,
			m_pointsColor, matchedPoints, trackedPoints, bothPoints,
			m_configFile.m_drawMatchingLines, out);
	}
	else
	{
		countKeypointsMatching(m_firstFrameKeypoints, m_updatedKeypoints, m_keypointStatus,
			matchedPoints, trackedPoints, bothPoints);
	}

	if (m_configFile.m_showVoting)
	{
		drawCentroidVotes(m_updatedKeypoints, m_centroidVotes, m_clusteredCentroidVotes,
			m_clusteredBorderVotes, m_keypointStatus, m_configFile.m_drawVotingLines,
			m_configFile.m_drawVotingFalse, out);
	}

	drawKeipointsStats(m_initKPCount, matchedPoints, trackedPoints, bothPoints, out);

	int totM = matchedPoints + bothPoints;
	int totT = trackedPoints + bothPoints;

	drawInformationHeader(m_numFrames, m_mScale, m_mAngle, m_clusterVoteSize, totM, totT, out);
...
//#ifdef _WIN64
void Tracker2::parDebugtrackingStep(const cv::Mat& fstFrame, const cv::Mat& scdFrame)
{
	random_device rd;
	default_random_engine engine(rd());
	uniform_int_distribution<unsigned int> uniform_dist(0, 255);
		...
		[&](const blocked_range<size_t>& r)
	{
		...
		{
			Scalar color(uniform_dist(engine), uniform_dist(engine), uniform_dist(engine));
			...
			if (s == Status::BOTH || s == Status::MATCH || s == Status::TRACK)
			{
				if (m_clusteredCentroidVotes[i])
				{
					circle(out, m_centroidVotes[i], 3, Scalar(0, 255, 0), -1);
					line(out, m_centroidVotes[i], scd, Scalar(0, 255, 0), 1);
				}
				else
				{
					circle(out, m_centroidVotes[i], 3, Scalar(0, 0, 255), -1);
					line(out, m_centroidVotes[i], scd, Scalar(0, 0, 255), 1);
				}
			...
			line(out, m_fstBBPoints[0] + offset, m_fstBBPoints[1] + offset, Scalar(255, 0, 0), 2);
			line(out, m_fstBBPoints[1] + offset, m_fstBBPoints[2] + offset, Scalar(255, 0, 0), 2);
			line(out, m_fstBBPoints[2] + offset, m_fstBBPoints[3] + offset, Scalar(255, 0, 0), 2);
			line(out, m_fstBBPoints[3] + offset, m_fstBBPoints[0] + offset, Scalar(255, 0, 0), 2);
	...
	if (m_initKPCount > 0)
	{
		nMatch = (windowHeight * (matchedPoints + bothPoints)) / m_initKPCount;
		nTrack = (windowHeight * (trackedPoints + bothPoints)) / m_initKPCount;
		nboth = (windowHeight * bothPoints) / m_initKPCount;

		cout << "nMatch " << nMatch << " nTrack " << nTrack << " nboth " << nboth << "\n";

		Rect mR(windowX, windowY + (windowHeight - nMatch), histWidth, nMatch);
		Rect tR(windowX + histWidth, windowY + (windowHeight - nTrack), histWidth, nTrack);
		Rect bR(windowX + 2 * histWidth, windowY + (windowHeight - nboth), histWidth, nboth);
...
void Tracker2::updateClusteredParams(float scale, float angle)
{
	scale = 1;
	angle = 0;
...
void Tracker2::printKeypointStatus(string filename, string message)
{
	...
	file << "BG " << bg << " init " << init << " match " << match << " track " << track
		<< " lost " << lost << " nomatch " << nomatch << "\n";

	file.close();
}

void Tracker2::drawResult(Mat& out)
{
...
void Tracker2::debugCalculations()
{
	...
	for (size_t i = 0; i < m_updatedKeypoints.size(); ++i)
	{
		for (size_t j = 0; j < m_firstFrameKeypoints.size(); j++)
		{
			if (isKeypointValid(i) && isKeypointValid(j) && i != j)
			{
				...
				Point2f b = m_firstFrameKeypoints[i].pt - m_firstFrameKeypoints[j].pt;
				Point2f c = a - b;
				angles.push_back(atan2(c.y, c.x));// - atan2(b.y, b.x));
				float nextDistance = getDistance(m_updatedKeypoints[i], m_updatedKeypoints[j]);
				float currDistance = getDistance(m_firstFrameKeypoints[i],
					m_firstFrameKeypoints[j]);
				...
				file << a.x << "," << a.y << "," << b.x << "," << b.y << ","
					<< atan2(c.y, c.x) << "," << (nextDistance / currDistance) << "\n";
			}
		}
	}
